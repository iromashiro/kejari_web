...
<div class="col-md-12 ml-auto mr-auto">
    <div class="card card-signup">
        <h2 class="card-title text-center">Index Pegawai</h2>
        <div class="col-md-11 ml-auto mr-auto">
            <a href="{{ route('form-tambah.pegawai') }}" type="button" class="btn btn-info mb-5">Input Pegawai
                Baru</a>
            <table id="example" class="display" style="width:100%">
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td><img src="{{ Storage::url($y->path) }}" width="50px" height="50px"></td>
                        <td>{{ $y->nip }}</td>
                        <td>{{ $y->nama }}</td>
                        <td>{{ $y->pendidikan_terakhir }}</td>
                        <td>{{ $y->golongan_pangkat }}</td>
                        @foreach ($y->jabatan as $j)
                        <td>{{ $j->nama_jabatan }}</td>
                        @endforeach
                        <td class="td-actions text-right">
                            <a href="{{ route('form-edit.pegawai', $y->id) }}" type="button" rel="tooltip"
                            ...
                            <form method="POST" action="{{route('delete.pegawai', $y->id)}}" class="d-inline-block">
                                @csrf
                                @method('delete')
